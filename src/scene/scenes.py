...
class Scene:
    ...
    def render(self) -> None:
        GLUtils.prepare_render(color = self.background_color)
...
class GLUtils:
    ...
    def draw_polygon(points: list, draw_points = True, *args, **kwargs) -> None:
        edge_color = kwargs.get("edge_color", (0.1, 0.1, 0.2, 1))
        vertex_color = kwargs.get("vertex_color", (1.0, 0.0, 0.0, 1.0))
        size = kwargs.get("size", 1)
        ...
        glEnd()
        if draw_points:
            GLUtils.draw_points(points, color = vertex_color)


class SvgScene(Scene):
    def __init__(self, title: str, svg: str, max_fps: int, **kwargs) -> None:
        image = Image.open(svg)
        h, w = image.size
        image = np.asarray(image.resize((h//5, w//5)))
        h, w = image.shape
        super().__init__(title, w, h, max_fps, **kwargs)
...
class DrawingScene(Scene):
    def __init__(self, title: str, width: int, height: int, max_fps: int, **kwargs) -> None:
